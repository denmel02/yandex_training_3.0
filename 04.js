...
const fn = (pupils, variants, table, side) => {
    if (variants >= pupils) {
        return [ -1 ];
    }

    const place = (table - 1) * 2 + side;
    const nextPlace = place + variants;
    const prevPlace = place - variants;

    switch (true) {
        case nextPlace <= pupils && prevPlace >= 1: {
            if (side === (nextPlace % 2 || 2)) {
                return [ Math.ceil(nextPlace / 2), side ];
            }

            return [ Math.ceil(prevPlace / 2), prevPlace % 2 || 2 ];
        }
        case nextPlace <= pupils:
            return [ Math.ceil(nextPlace / 2), nextPlace % 2 || 2 ];
        case prevPlace >= 1:
            return [ Math.ceil(prevPlace / 2), prevPlace % 2 || 2];
        default:
            return [ -1 ];
...
console.log(fn(25, 2, 1, 2).join(' '));
console.log(fn(25, 13, 7, 1).join(' '));
